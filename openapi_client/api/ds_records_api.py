...
class DsRecordsApi(object):
    ...
    def __init__(self, api_client=None):
        ...
        self.zones_zone_id_ds_records_get_endpoint = _Endpoint(
            settings={
                'response_type': (GetDsRecords,),
                'auth': [],
                'endpoint_path': '/zones/{ZoneId}/ds_records',
                'operation_id': 'zones_zone_id_ds_records_get',
                'http_method': 'GET',
                'servers': None,
            },
            params_map={
    ...
    def zones_zone_id_ds_records_get(
        self,
        zone_id,
        **kwargs
    ):
        """DSレコードの一覧取得  # noqa: E501

        上位のネームサーバに登録が必要なDSレコードを取得できます。  ### Authorizations dpf_read   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True

        >>> thread = api.zones_zone_id_ds_records_get(zone_id, async_req=True)
        >>> result = thread.get()
        ...
        kwargs['zone_id'] = \
            zone_id
        return self.zones_zone_id_ds_records_get_endpoint.call_with_http_info(**kwargs)
