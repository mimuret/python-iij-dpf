...
class JobsApi(object):
    ...
    def __init__(self, api_client=None):
        ...
        self.jobs_request_id_get_endpoint = _Endpoint(
            settings={
                'response_type': (GetJobs,),
                'auth': [],
                'endpoint_path': '/jobs/{RequestId}',
                'operation_id': 'jobs_request_id_get',
                'http_method': 'GET',
                'servers': None,
            },
            params_map={
    ...
    def jobs_request_id_get(
        self,
        request_id,
        **kwargs
    ):
        """非同期リクエストの状態確認  # noqa: E501

        ジョブの進捗状況を取得します。\\ このAPIの詳しい利用方法は、**[非同期リクエスト](#section/利用方法/非同期リクエスト)**を参照してください。  ### Authorizations dpf_read   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True

        >>> thread = api.jobs_request_id_get(request_id, async_req=True)
        >>> result = thread.get()
        ...
        kwargs['request_id'] = \
            request_id
        return self.jobs_request_id_get_endpoint.call_with_http_info(**kwargs)
