...
class ContractsLogsStatus(ModelSimple):
    ...
    def __init__(self, *args, **kwargs):
        """ContractsLogsStatus - a model defined in OpenAPI

        Note that value can be passed either in args or in kwargs, but not in both.

        Args:
            args[0] (str): 詳細説明は**[こちら](#tag/logs-(contracts))**., must be one of ["start", "success", "failure", "retry", ]  # noqa: E501

        Keyword Args:
            value (str): 詳細説明は**[こちら](#tag/logs-(contracts))**., must be one of ["start", "success", "failure", "retry", ]  # noqa: E501
