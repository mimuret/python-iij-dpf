...
class RecordsRdata(ModelSimple):
    ...
    def __init__(self, *args, **kwargs):
        """RecordsRdata - a model defined in OpenAPI

        Note that value can be passed either in args or in kwargs, but not in both.

        Args:
            args[0] ([{str: (bool, date, datetime, dict, float, int, list, str, none_type)}]): レコードの値.  # noqa: E501

        Keyword Args:
            value ([{str: (bool, date, datetime, dict, float, int, list, str, none_type)}]): レコードの値.  # noqa: E501
