...
class SearchType(ModelSimple):
    ...
    def __init__(self, *args, **kwargs):
        """SearchType - a model defined in OpenAPI

        Note that value can be passed either in args or in kwargs, but not in both.

        Args:
            args[0] (str): if omitted defaults to "AND", must be one of ["AND", "OR", ]  # noqa: E501

        Keyword Args:
            value (str): if omitted defaults to "AND", must be one of ["AND", "OR", ]  # noqa: E501
